...
function removeFlavorByName(array, removalFlavor) {
  const flavorIndex = array.findIndex((indexValue) => {
    indexValue === removalFlavor;
...
function filterByWord(array, filterFlavor) {
  let foundFlavors = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].includes(filterFlavor)) {
      foundFlavors.push(array[i]);
    }
  }
  return foundFlavors;
}
